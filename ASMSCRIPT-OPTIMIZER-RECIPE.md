...
JavaScript(ホスト)とWebAssembly(ゲスト)間でデータを交換する際の必須ルールです。AIはユーザーの要求を分析し、以下のパターンから最適なものを選択して、必要なヘルパー関数を`AppModule`内に実装してください。

### **パターン1: プリミティブ型 (数値, 真偽値)**
* **ルール**: 関数の引数・戻り値として**直接受け渡し**します。特別な処理は不要です。
* **対象**: `i32`, `f64`, `bool` など。
* **例**: `export function add(a: i32, b: i32): i32 { return a + b; }`

### **パターン2: 連続データ (文字列, 型付き配列)**
* **ルール**: パフォーマンスが重要なため、**ポインタ渡し**を原則とします。データのコピーは最小限に留めてください。AssemblyScript側のメモリ確保/解放には `__new` と `__pin` / `__unpin` を適切に使用します。

* #### **文字列 (String)**
  * **JS → AS**: JavaScript側で `TextEncoder` を使用してUTF-8にエンコードし、WebAssemblyのメモリに書き込みます。AS側にはその**アドレス(ポインタ)**と**バイト長**を渡します。
  * **AS → JS**: AS側は文字列のポインタとバイト長を返します。JavaScript側で `TextDecoder` を使用してメモリからその領域を読み出し、デコードします。

* #### **型付き配列 (TypedArray)**
  * **JS → AS**: AS側でメモリ確保関数 (`__new`) を呼び出して得られたポインタに対し、JavaScript側から `(u)int8Array.set()` などでデータを直接書き込みます。
  * **AS → JS**: AS側は配列のポインタを返します。JS側では `new Float32Array(memory.buffer, ptr, len)` のようにメモリビューを作成してデータを直接読み取ります。

### **パターン3: 構造化データ (オブジェクト, 汎用配列)**
* **ルール**: 直接渡せないため、**JSON文字列へのシリアライズ**を必須とします。
* **対象**: `{ id: 1 }` のようなオブジェクト、`[1, "a"]` のような配列。
* **手順**:
  1. JavaScript側で `JSON.stringify()` を使用してオブジェクトを文字列に変換します。
  2. 変換後のJSON文字列を、**パターン2の文字列受け渡しルール**に従ってASに渡します。
  3. (必要な場合) AS側でJSONをパースします。
     * **注記**: AS側での複雑なパースが必要な場合、AIは`json-as`のような外部ライブラリの利用を検討し、その旨をコメントで注記してください。（※ただし、本レシピの単一ファイル構成ではライブラリの直接利用は困難なため、プリミティブな実装を優先します）
