...
const CDN_PROVIDERS = [
    ...
    },
    {
        ...
        characteristics: 'モダンESM配信・実験的・最終フォールバック'
    }
];
```
...
1. **直接importは絶対に使用しない**
2. **必ずweb.js経由でロード**
3. **CDNはjsDelivr、UNPKG、esm.shの3つ**（優先順位順）
4. **import mapの設定を待機**
...
| **UNPKG** | unpkg.com | npm公式ミラー・完全性保証 | 第2選択 |
| **esm.sh** | esm.sh | モダンESM配信・実験的 | 最終フォールバック |
...
- ✅ **初期化時間**: 最悪ケース30秒（3CDN × 10秒タイムアウト）

**技術的改善：**
- esm.shをpriority:3として追加
- プログレスバー計算を3CDN用に調整
- v1.3.1で除外したesm.shを有用と判断し復活
